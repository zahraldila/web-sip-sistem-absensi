...
@section('content')
<div x-data="{ filterOpen: false, exportModalOpen: false, exportFormat: 'excel' }" class="space-y-6">
    ...
    <section>
        <div class="mb-8">
            ...
            <p class="mt-2 text-[15px] text-slate-500">
                Ringkasan Laporan Kehadiran Karyawan secara menyeluruh
            </p>
        </div>

        <form method="GET" action="{{ route('admin.laporan-kehadiran') }}" class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-end">
            <div class="flex w-full flex-wrap items-center gap-2 lg:justify-end">
                <div class="relative flex-1 sm:w-[280px]">
                    ...
                    <input
                        ...
                        class="w-full rounded-3xl border border-slate-200 bg-white pl-10 pr-4 py-3 text-sm text-slate-900 placeholder-slate-400 shadow-sm outline-none transition focus:border-slate-400 focus:ring-2 focus:ring-slate-200" />
                </div>
                ...
                <div class="relative">
                    <button
                        ...
                        class="inline-flex items-center justify-center gap-2 rounded-3xl border border-slate-200 bg-white px-6 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-50 whitespace-nowrap">
                    ...
                    <div x-show="filterOpen" x-transition x-cloak class="absolute right-0 top-full z-20 mt-2 w-[320px] rounded-3xl border border-slate-200 bg-white p-4 shadow-xl">
                        <div class="space-y-3">
                        ...
                        <div class="mt-4 flex items-center justify-end gap-2 border-t border-slate-200 pt-3">
                            ...
                            <button
                                type="submit"
                                class="rounded-2xl bg-[#123D91] px-4 py-2 text-sm font-semibold text-white transition hover:bg-[#0E337A]">
                                Terapkan
                            </button>
                        </div>
                    </div>
                ...
                <button
                    ...
                    class="inline-flex items-center justify-center gap-2 rounded-3xl bg-[#123D91] px-6 py-3 text-sm font-semibold text-white transition hover:bg-[#0E337A] whitespace-nowrap">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <section class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-card">
        <div class="overflow-x-auto">
            <table class="min-w-full border-separate border-spacing-0 text-left text-sm">
                <thead class="bg-slate-50">
                    <tr>
                        <th class="px-6 py-4 font-semibold uppercase tracking-[0.12em] text-slate-600 text-xs">Karyawan</th>
                        <th class="px-6 py-4 font-semibold uppercase tracking-[0.12em] text-slate-600 text-xs">Tanggal</th>
                        <th class="px-6 py-4 font-semibold uppercase tracking-[0.12em] text-slate-600 text-xs">Masuk</th>
                        <th class="px-6 py-4 font-semibold uppercase tracking-[0.12em] text-slate-600 text-xs">Keluar</th>
                        <th class="px-6 py-4 font-semibold uppercase tracking-[0.12em] text-slate-600 text-xs">Durasi</th>
                        <th class="px-6 py-4 font-semibold uppercase tracking-[0.12em] text-slate-600 text-xs">Mode</th>
                        <th class="px-6 py-4 font-semibold uppercase tracking-[0.12em] text-slate-600 text-xs">Lokasi</th>
                        <th class="px-6 py-4 font-semibold uppercase tracking-[0.12em] text-slate-600 text-xs">Status</th>
                    </tr>
                </thead>
                        ...
                        <tr class="transition hover:bg-slate-50">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="min-w-0">
                                        <p class="truncate text-sm font-medium text-slate-900">{{ $attendance->pegawai?->nama_pegawai ?? '-' }}</p>
                                    </div>
                                </div>
                            </td>

                            <td class="px-6 py-4 text-sm text-slate-700">{{ \Carbon\Carbon::parse($attendance->tanggal_absensi)->translatedFormat('d F Y') }}</td>
                            <td class="px-6 py-4 text-sm text-slate-700">{{ $attendance->jam_checkin ? \Carbon\Carbon::parse($attendance->jam_checkin)->format('H:i') : '-' }}</td>
                            <td class="px-6 py-4 text-sm text-slate-700">{{ $attendance->jam_checkout ? \Carbon\Carbon::parse($attendance->jam_checkout)->format('H:i') : '-' }}</td>
                            ...
                            <td class="px-6 py-4 text-sm text-slate-700">{{ $attendance->skema_kerja ?? '-' }}</td>
                            <td class="px-6 py-4 text-sm text-slate-700">{{ $attendance->latitude !== null && $attendance->longitude !== null ? $attendance->latitude . ', ' . $attendance->longitude : '-' }}</td>
        ...
        <div class="flex flex-col gap-4 border-t border-slate-200 px-6 py-5 md:flex-row md:items-center md:justify-between">
            <p class="text-sm text-slate-600">
                Showing {{ $attendances->firstItem() ?? 0 }} to {{ $attendances->lastItem() ?? 0 }} of {{ $attendances->total() }} employees
            </p>
            <div class="flex items-center gap-3">
                @if($attendances->onFirstPage())
                    <span class="inline-flex items-center justify-center rounded-2xl border border-slate-200 bg-slate-50 px-4 py-2 text-sm font-medium text-slate-400 cursor-not-allowed">
                        Prev
                    </span>
                @else
                    <a href="{{ $attendances->withQueryString()->previousPageUrl() }}" class="inline-flex items-center justify-center rounded-2xl border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-50">
                        Prev
                    </a>
                @endif

                @if($attendances->hasMorePages())
                    <a href="{{ $attendances->withQueryString()->nextPageUrl() }}" class="inline-flex items-center justify-center rounded-2xl border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-50">
                        Next
                    </a>
                @else
                    <span class="inline-flex items-center justify-center rounded-2xl border border-slate-200 bg-slate-50 px-4 py-2 text-sm font-medium text-slate-400 cursor-not-allowed">
